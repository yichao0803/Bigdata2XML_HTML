...
class ldr2HbaseInterface:
    ...
    def __init__(self,_connstr,emrConnstr,patient_id,visit_id,patient_name,year):
        self._connstr=_connstr
    ...
    def saveHtmlFile4Ldr(self,topic,mr_code,mr_name,_time,html):
        htmlDir=(self._pathHtml4LdrDir+self.patient_id[len(self.patient_id)-2:len(self.patient_id)]+"/"+self.patient_id[0:len(self.patient_id)-2].zfill(8)+"/")
    ...
    def saveHtmlFile(self,topic,mr_code,mr_name,_time,html):
        ...
        fileName=topic+self._split+mr_code+self._split+mr_name+self._split+time.strftime('%Y-%m-%d %H_%M_%S', timestruct)+".html"
    ...
    def getHttp(self,httpUrl):
        ...
        if unConten.find('<meta http-equiv="content-type" content="text/html;charset=utf-8">')<0:
            if unConten.find('<title></title>')>0:
              unConten=unConten.replace('<title></title>','<title></title>\n<meta http-equiv="content-type" content="text/html;charset=utf-8">')
            elif unConten.find(u'<title>首都医科大学附属北京天坛医院</title>')>0:
              unConten=unConten.replace(u'<title>首都医科大学附属北京天坛医院</title>',u'<title>首都医科大学附属北京天坛医院</title>\n<meta http-equiv="content-type" content="text/html;charset=utf-8">')
        return unConten
                ...
                if line.find("#")<0:
                    ...
                    if tup[1]=="":           
                        ...
                        if tup[1]=="REPORT_DATE_TIME":
                            _sql+=(",to_char(to_date(to_char(lr.report_date_time,'yyyy-mm-dd')||' '||replace(lpad(lr.order_no,4),' ','0')||'00','yyyy/mm/dd hh24miss'),'yyyy-mm-dd hh24:mi:ss') as "+ tup[0]) 
                        else:                   
                            _sql+=(",t."+tup[1]+" as "+ tup[0]) 
                    _columns.append(tup[0]);        
            ...
            if line.find("#")<0:
                ...
                if tup[1]=="":           
                    ...
                    if tup[1]=="DISCHARGE_PASS":
                        _sql+=", case v.DISCHARGE_PASS when '1' then '医嘱离院'when '2' then '医嘱转院'when '3' then '医嘱转社区'when '4' then '非医嘱离院'when '5' then '死亡'when '9' then '其他' else '其他' end as"+ tup[0]
                    else:
                        _sql+=(", "+tup[1]+" as "+ tup[0]) 
                _columns.append(tup[0]);        
    ...
    order by d.class_code asc"""
        ...
        _sql="""select distinct d.class_code,d.class_name,d.class_childname
      from  dqmc_local_sas_dict d 
    left join x_laid_up_standard t  on ( d.map_name like  t.name or d.map_fullname like t.name)
    inner join x_laid_file f on f.patient_id=t.patient_id and f.visit_id=t.visit_id and f.file_no=t.file_no and f.file_name=t.file_name and f.flag_sccf=0 and f.file_flag>='1' and file_flag!='T'
    left join x_laid_time lt on d.sas_name=lt.date_code        
    ...
    order by d.class_code,class_childname asc"""
        ...
        _sql="""select distinct d.class_code,d.class_name,d.class_childname,t.name,t.values_name,
        lt.time_name
    from  dqmc_local_sas_dict d 
    left join x_laid_up_standard t  on ( d.map_name like  t.name or d.map_fullname like t.name)
    inner join x_laid_file f on f.patient_id=t.patient_id and f.visit_id=t.visit_id and f.file_no=t.file_no and f.file_name=t.file_name and f.flag_sccf=0 and f.file_flag>='1' and file_flag!='T'
    ...
    order by d.class_code,d.class_childname asc"""
        ...
        for index,row in classData.iterrows():
            ...
            for j,rowc in (class_childnameData[['class_childname']][class_childnameData["class_code"]==Utility.toStr(row['class_code'])]).iterrows():
                ...
                for k,rowv in (values_nameData[values_nameData['class_childname'].isin([Utility.toStr(rowc['class_childname'])])]).iterrows():
                     ...
                     items+=self._value % (Utility.tNodeXml(Utility.toStr(rowv["name"])),Utility.txml(x_varlue),Utility.tNodeXml(Utility.toStr(rowv["name"])))
    ...
    def toHtmlfile(self,connstrEmr,htmlUrlBase):
        ...
        for i,row in pd.DataFrame(rows,columns=["PATIENT_ID","VISIT_ID","FILE_NO","FILE_NAME","TOPIC","LAST_MODIFY_DATE_TIME","MR_CODE"]).iterrows():  
            htmlUrl=htmlUrlBase+"?patient_id="+Utility.toStr(row["PATIENT_ID"])+"&visit_id="
            htmlUrl+=Utility.toStr(row["VISIT_ID"])+ "&file_no="
            htmlUrl+=Utility.toStr(row["FILE_NO"])+"&file_name="+Utility.toStr(row["FILE_NAME"])+"&topic="+quote(Utility.toStr(row["TOPIC"]))+"&patient_name="+quote(self.patient_name)
            try:                 
                if Utility.toStr(row["TOPIC"]).find("会商")>0:
                    html=self.getHuiShang(Utility.toStr(row["FILE_NAME"]))
                    if saveHtml4Dashuju:
                        self.saveHtmlFile(Utility.toStr(row["TOPIC"]),Utility.toStr(row["MR_CODE"]),Utility.toStr(row["FILE_NAME"]),Utility.toStr(row["LAST_MODIFY_DATE_TIME"]),html)
                    if saveHtml4Ldr:
                        self.saveHtmlFile4Ldr(Utility.toStr(row["TOPIC"]),Utility.toStr(row["MR_CODE"]),Utility.toStr(row["FILE_NAME"]),Utility.toStr(row["LAST_MODIFY_DATE_TIME"]),html)
                else:
...
class handlePatVisit:
    '根据不同的患者集处理信息'
    @staticmethod 
    def patVisit(connstr,emrConnstr,htmlUrlBase,isSaveXml,isSaveHtml,pid,vid):
        sql="""select t.patient_id,t.visit_id,M.PATIENT_NAME from dqmc_pat_visit t
        left join dqmc_pat_master m on T.PATIENT_ID=M.PATIENT_ID where 
        t.patient_id='"""+pid+"""' and t.visit_id='"""+vid+"""' order by t.admission_date_time desc"""
        rows=oracleHelper.fetchall(connstr,sql)
        for i,row in pd.DataFrame(rows,columns=["patient_id","visit_id","PATIENT_NAME"]).iterrows():
            lhi=ldr2HbaseInterface(connstr,emrConnstr,Utility.toStr(row["patient_id"]),Utility.toStr(row["visit_id"])
            ,Utility.toStr(row["PATIENT_NAME"]),"Test")
            if isSaveXml:               
                try:
                    lhi.toXmlfile(emrConnstr)
                except Exception,e:
                    logger.error(Utility.toStr(row["patient_id"])+"#"+Utility.toStr(row["visit_id"])+"#"+Utility.toStr(row["PATIENT_NAME"]))
                    logger.error('str(Exception):'+str(Exception))
                    logger.error('str(e):'+str(e))
                    logger.error('repr(e):'+repr(e))
                    logger.error('e.message:'+e.message)                  
    ...
    @staticmethod
    def patVisit4Time(connstr,emrConnstr,htmlUrlBase,isSaveXml,isSaveHtml,processescount):
        dBegin=datetime.datetime.strptime("2009-01-01 00:00:00","%Y-%m-%d %H:%M:%S")
        dEnd=datetime.datetime.strptime("2009-12-31 23:59:59","%Y-%m-%d %H:%M:%S")
        ...
        for k in range(0,(dEnd.year-dBegin.year)+1):  
            ...
            sql+=d1.strftime("%Y-%m-%d %H:%M:%S")+"','yyyy-mm-dd hh24:mi:ss') and  to_date('"+d2.strftime("%Y-%m-%d %H:%M:%S")+"','yyyy-mm-dd hh24:mi:ss') order by t.admission_date_time desc"
            logger.info(sql)
            rows=oracleHelper.fetchall(connstr,sql)
            ...
            for i,row in dataR.iterrows():
                ...
                if isSaveHtml:
                    pool.apply_async(patVisit4Time2Html, (connstr,emrConnstr,htmlUrlBase,Utility.toStr(row["patient_id"]),Utility.toStr(row["visit_id"]),Utility.toStr(row["PATIENT_NAME"]),Utility.toStr(dBegin.year+k), ))             
        pool.close()
        pool.join()   #调用join之前，先调用close函数，否则会出错。执行完close后不会有新的进程加入到pool,join函数等待所有子进程结束
 
            
def patVisit4Time2Xml(connstr,emrConnstr,patient_id,visit_id,PATIENT_NAME,years):
    #logger.info(patient_id+"#"+visit_id)
    try:
        lhi=ldr2HbaseInterface(connstr,emrConnstr,patient_id,visit_id,PATIENT_NAME,years)   
        lhi.toXmlfile(emrConnstr) 
    except Exception,e:
...
def patVisit4Time2Html(connstr,emrConnstr,htmlUrlBase,patient_id,visit_id,PATIENT_NAME,years):
    try:
        lhi=ldr2HbaseInterface(connstr,emrConnstr,patient_id,visit_id,PATIENT_NAME,years)   
        lhi.toHtmlfile(emrConnstr,htmlUrlBase) 
    except Exception,e:
