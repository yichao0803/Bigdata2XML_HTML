#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Created by ZhangYichao on 2018/9/14
...
def like_user():
    ...
    cmd = 'shell input tap {x} {y}'.format(
        x=config['like_bottom']['x'] + _random_bias(1),
        y=config['like_bottom']['y'] + _random_bias(1)
    )
    adb.run(cmd)
    time.sleep(0.5)
...
def dislike_user():
    ...
    """
    cmd = 'shell input tap {x} {y}'.format(
        x=config['dislike_bottom']['x'] + _random_bias(1),
        y=config['dislike_bottom']['y'] + _random_bias(1)
    )
    adb.run(cmd)
    time.sleep(0.5)
